    ...
    return (
      <div className="min-h-screen bg-[#0B0F19] flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center">
          <h1 className="text-3xl font-bold text-white mb-2 tracking-tight">PoliTrack <span className="text-tenant-primary">AI</span></h1>
          <p className="text-zinc-500 mb-8 uppercase text-xs font-bold tracking-widest">Intelligence for Victory</p>
          <button 
            onClick={() => setIsLoggedIn(true)}
            className="w-full py-4 bg-tenant-primary text-white font-bold rounded-xl hover:opacity-90 transition-all flex items-center justify-center gap-2"
          >
  ...
  return (
    <div className="relative">
      ...
      {isPublicMicrosite ? (
        ...
        <DashboardLayout user={user} tenant={tenant} onUpdateTenant={handleUpdateTenant} />
      )}
    </div>
  );
}
